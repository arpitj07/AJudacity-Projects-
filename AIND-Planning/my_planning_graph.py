...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #THANKS TO MENTOR FOR HELPING WITH THIS METHOD
        self.a_levels.append(set())   # A0 set of a_nodes - empty to start
            ...
            if a_node.prenodes.issubset(self.s_levels[level]):
                ...
                for s_node in self.s_levels[level]:
                    ...
                    if s_node in a_node.prenodes:
                        ...
                        s_node.children.add(a_node)  #connecting action node to S node 
                        a_node.parents.add(s_node)   #connectiing S node to action node
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level-1]:
            for effects in a_node.effnodes:
                a_effect.append(a_node)
                for a_node in a_effect:

                    a_node.children.add(effects)
                    effects.parents.add(a_node)
                

                    self.s_levels[level].add(effects)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        return (set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)) | (set(node_a2.action.effect_add) & set(node_a1.action.effect_rem))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        node1_pos= set(node_a1.action.precond_pos)
        node2_pos= set(node_a2.action.precond_pos)

        node1_neg= set(node_a1.action.precond_neg)
        node2_neg= set(node_a2.action.precond_neg)
        ...
        return (node1_add & node2_neg)|(node2_add & node1_neg)|(node1_rem & node2_pos)|(node2_rem & node1_pos)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        return all(node1.is_mutex(node2) for node1 in node_s1.parents  for node2 in node_s2.parents)
    ...
    def h_levelsum(self) -> int:
        ...
        #THANKS TO MENTOR FOR HELPING WITH THIS HEURISTIC 

        level_sum = 0
        for goal in self.problem.goal:
            node_goal=PgNode_s(goal,True)
            for level, states in enumerate(self.s_levels):
                
                if node_goal in states:
                    level_sum +=level
                    break
